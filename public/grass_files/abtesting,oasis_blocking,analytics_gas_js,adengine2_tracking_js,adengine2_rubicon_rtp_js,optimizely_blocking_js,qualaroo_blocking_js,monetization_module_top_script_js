...
(function(window){var jQueryRequested=false;window.getJqueryUrl=function(){var result=[];if((typeof window.jQuery==='undefined'||typeof window.jQuery.fn==='undefined')&&jQueryRequested==false){jQueryRequested=true;result=result.concat(window.wgJqueryUrl);}
...
define('ext.wikia.adEngine.adTracker',['wikia.tracker','wikia.window'],function(tracker,window){'use strict';var timeBuckets=[0.0,0.5,1.0,1.5,2.0,2.5,3.5,5.0,8.0,20.0,60.0];function encodeAsQueryString(extraParams){var out=[],key;for(key in extraParams){if(extraParams.hasOwnProperty(key)){out.push(key+'='+extraParams[key]);}}
return out.join(';');}
...
define('ext.wikia.adEngine.rubiconRtp',['ext.wikia.adEngine.adTracker','wikia.document','wikia.log','wikia.window'],function(adTracker,document,log,window){'use strict';var logGroup='ext.wikia.adEngine.rubiconRtp',timingEventData={ozCachedOnly:!!window.wgAdDriverRubiconCachedOnly},rtpTiming,rubiconCalled=false,rtpResponse,rtpTier;function trackState(trackEnd){log(['trackState',rtpResponse],'debug',logGroup);var e='(unknown)',eventName,data={},label;data.ozCachedOnly=!!window.wgAdDriverRubiconCachedOnly;data.response=!!rtpResponse;data.size=(rtpResponse&&rtpResponse.estimate&&rtpResponse.estimate.size)||e;data.pmpEligible=(rtpResponse&&rtpResponse.pmp&&rtpResponse.pmp.eligible)||e;data.tier=rtpTier||e;if(trackEnd){eventName='lookupEnd';}else if(rtpTier){eventName='lookupSuccess';}else{eventName='lookupError';}
...
function call(){log('call','debug',logGroup);rubiconCalled=true;rtpTiming=adTracker.measureTime('rubicon',timingEventData,'start');rtpTiming.track();window.oz_callback=onRubiconResponse;var s=document.createElement('script');s.src='//tap-cdn.rubiconproject.com/partner/scripts/rubicon/dorothy.js?pc='+window.oz_site;s.async=true;document.body.appendChild(s);}
...
return{call:call,getTier:getTier,trackState:function(){trackState();},wasCalled:wasCalled};});;
